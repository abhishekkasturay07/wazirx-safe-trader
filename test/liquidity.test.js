...
test('liquidity gate rejects wide spreads, low volume, shallow books, and unapproved symbols', () => {
  const healthyTicker = { bidPrice: '100', askPrice: '100.2', lastPrice: '100', volume: '200000' };
  const deepBook = { bids: [['100', '10']] };
  assert.equal(liquidityCheck('btcinr', healthyTicker, deepBook, 500).allowed, true);
  assert.match(liquidityCheck('btcinr', { ...healthyTicker, askPrice: '101' }, deepBook, 500).reason, /spread/);
  assert.match(liquidityCheck('btcinr', { ...healthyTicker, volume: '10' }, deepBook, 500).reason, /volume/);
  assert.match(liquidityCheck('btcinr', healthyTicker, { bids: [['100', '1'], ['90', '10']] }, 500).reason, /slippage/);
  assert.match(liquidityCheck('dogeinr', healthyTicker, deepBook, 500).reason, /allowlist/);
});
